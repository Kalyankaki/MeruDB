...
const providerIcons: Record<string, React.ReactNode> = {
  ...
  "azure-ad": (
    <svg className="h-5 w-5" viewBox="0 0 24 24">
      <path
        fill="currentColor"
        d="M5.483 21.3H24L14.025 4.013l-3.038 8.347 5.836 6.938L5.483 21.3zM13.049 2.7L0 21.3h5.483l7.566-11.262L13.049 2.7z"
      />
    </svg>
  ),
};
    ...
    <div className="min-h-screen flex items-center justify-center bg-background math-pattern p-4">
      ...
      <Card className="w-full max-w-md card-magical relative z-10">
        <CardHeader className="text-center">
          ...
          <CardDescription>
            Sign in to continue your journey as a Mathemagician
          </CardDescription>
        </CardHeader>
            ...
            <>
              {Object.values(providers).map((provider) => (
                <Button
                  key={provider.id}
                  variant="outline"
                  className="w-full h-12 text-base border-border hover:bg-violet-500/10 hover:border-violet-500/50"
                  onClick={() => handleSignIn(provider.id)}
                  disabled={isLoading !== null}
                >
                  {isLoading === provider.id ? (
                    <div className="h-5 w-5 animate-spin rounded-full border-2 border-violet-500 border-t-transparent mr-3" />
                  ) : (
                    <span className="mr-3">{providerIcons[provider.id]}</span>
                  )}
